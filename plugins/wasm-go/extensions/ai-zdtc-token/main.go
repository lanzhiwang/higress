...
const (
	...
	// ctxKeySSEFramer 保存当前请求的 SSE 事件分帧器 (见 sse_framer.go).
	ctxKeySSEFramer = "sse_framer"
)
...
func onHttpStreamResponseBody(ctx wrapper.HttpContext, config PluginConfig, chunk []byte, isLastChunk bool, log log.Log) []byte {
	...
	// SSE 事件分帧.
	// 一个 onHttpStreamResponseBody 回调收到的是任意字节块, 不保证是完整的 SSE 事件.
	// 直接对原始块调用 GetTokenUsage 会落入 tokenusage 内部按字段顺序猜切点的启发式
	// (mergeLargeResponseAPIChunks): 它只在 "response.completed" 出现在 "usage" 之前时
	// 才启动拼接, 因此当上游把 "type" 放在事件末尾时 (例如 Vendor3/DeepSeek-V4.1-Flash),
	// usage 一旦被回调边界切开就永久丢失, 导致 totalToken=0 并被误判为"流式传输异常中断".
	// 这里先把回调字节按 SSE 分隔符还原成完整事件, 再交给各消费方解析.
	framer := getSSEFramer(ctx)
	for _, event := range framer.frameCallback(chunk) {
		// 分帧后再探测一次 [DONE]: 修复 [DONE] 帧被回调边界切开导致的漏判
		if checkHasDoneMarker(event) {
			ctx.SetContext(ctxKeyHasSeenDone, true)
		}

		// 标准 Token 消耗解析 (提取 input_token, output_token, total_token, model)
		if usage := tokenusage.GetTokenUsage(ctx, event); usage.TotalToken > 0 {
		...
		if cachedTokens := extractCachedTokens(event); cachedTokens > 0 {
	...
	if isLastChunk {
		...
		// 未以分隔符结尾的残留事件不可恢复, 在此丢弃; 若残留曾超过上限则会进入 RESYNC 并被计数.
		if overflowCount := framer.drain(); overflowCount > 0 {
			log.Debugf("[ai-zdtc-token onHttpStreamResponseBody] SSE 分帧器丢弃了 %d 个超长未完整事件", overflowCount)
		}
...
// getSSEFramer 返回当前请求的 SSE 事件分帧器, 首次调用时创建并存入上下文.
func getSSEFramer(ctx wrapper.HttpContext) *sseFramer {
	if v := ctx.GetContext(ctxKeySSEFramer); v != nil {
		if framer, ok := v.(*sseFramer); ok && framer != nil {
			return framer
		}
	}
	framer := &sseFramer{}
